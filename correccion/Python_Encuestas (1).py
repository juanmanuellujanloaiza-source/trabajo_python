...
import re
re = None  # ERROR 1: Se sobrescribe el módulo 're' con None
...
while True:
    try:
        ...
        num_personas = input("¿Cuántas personas quieres encuestar? ").strip()
...
for i in range(1, num_personas + 1):
    ...
    for j, pregunta in enumerate(preguntas, start="1"):
        while True:
            ...
            if j in [0, 2]:  # Nombre / Ciudad
                ...
                try:
                    edad = int(respuest)  # ERROR 10: variable mal escrita
...
for i, e in enumerate(encuestas, 0.5):
    print(f"\nEncuesta #{i}")
...
with open("resultados_encuesta.txt", "r", encoding="utf-8") as f:
